...
<h1 align="center">Organisation des rencontres entre 2 equipes</h1>
    ...
    <?php
      ...
      if (isset ($_GET['afficherencontre'])){
        ...
        echo "<br/><br/>";
        /*var_dump($idT);
        // On va chercher en base le nombre de matchs enregistrés pour ce tournoi, et si ce nombre est > 0, inutile de refaire l'insertion en base.
        $sql = "SELECT COUNT(r.id_match) FROM ParticipMatch p LEFT JOIN Rencontre r ON r.id_match = p.idMatch WHERE idTournoi = $idT";
        $sth = $dbh->query($sql);
        if (!$sth) {
          die("impossible d'éxécuter la requête !");
        }
        $nombreDeRencontres = $sth->fetch();
        ...
        if ($nombreDeRencontres === 0) {*/
          ...
          $schedule = scheduler($teams,1,1);
          $IDT = ($_SESSION['currentTournoi']['nbEquipe']/2);
          //var_dump($schedule);
          foreach($schedule as $nom){
              //$sousTableau = array_chunk($nom,1);
              //var_dump($nom);
              $jour = $jour + 1;
              echo "<br/><strong>Jour : ";
              echo $jour;
              ...
              $SousTabs = array_chunk($nom,1);
              //var_dump($sousTableau);
              foreach( $SousTabs as $SousTab){
                //var_dump($SousTab); //$soutab c'est les match la reunion de deux equipes qui s'affronte
                ...
                echo "<ul>";
                //echo $soutab['Home']."-".$soutab['Away']."<br>";
                ...
                foreach ($SousTab as $Equipe){
                  //var_dump($Equipe);
                  $home = $Equipe['Home'];
                  //var_dump($home);
                  $away = $Equipe ['Away'];
                
                  $nomEquipe = "SELECT id_equipe, nomEquipe FROM Equipe WHERE id_equipe IN ($home, $away);";
                  //var_dump($nomEquipe);
                  $info = $dbh->query($nomEquipe);
                  $equipes = $info->fetchAll();
                  if (!$equipes) die("Impossible d'éxécuter la requête !");
                  //var_dump($equipes);
                  foreach ($equipes as $equipe){
                    if ($equipe['id_equipe'] === $home) {
                      $homeName = $equipe['nomEquipe'];
                    } else {
                      $awayName = $equipe['nomEquipe'];
                    }
                    //insertion de table LISTEEQUIPE pour lier les match avec les equipes et attribuer les score à chacun
                    $sql = "INSERT INTO ListeEquipe (idMatch, idEquipe, score) VALUES (".$_SESSION ['currentMatch']['id_match'].", ".$equipe['id_equipe'].", NULL);";
                    //execution
                    $sth = $dbh->prepare($sql);
                    $sth->execute();                
                  }
        ...
        // On va chercher en base la liste des rencontres et journées et tout ca et on l'affiche a l'écran
      }
